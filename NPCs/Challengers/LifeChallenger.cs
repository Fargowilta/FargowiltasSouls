        ...
        public void P1ShotSpam()
        {
            ...
            if (NPC.ai[2] > 60)
            {
                ...
                if (++NPC.localAI[1] % threshold == 0)
                {
                    ...
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel.RotatedBy(rotationToUse * i), projType, FargoSoulsUtil.ScaledProjectileDamage(NPC.damage), 0f, Main.myPlayer/*, NPC.localAI[1] + (1000*i)*/);
                    }
                }
            }
            ...
            if (NPC.ai[1] > 60f && NPC.ai[1] < 360f)
            {
                ...
                if (NPC.ai[2] > 1)
                {
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        float ai1 = -1;
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, new Vector2(0f, 7f), ModContent.ProjectileType<LifeProjSmall>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.damage), knockBack6, Main.myPlayer, 0, ai1);
                    }
                    NPC.ai[2] = 0f;
                }
                NPC.ai[2] += 1f;
