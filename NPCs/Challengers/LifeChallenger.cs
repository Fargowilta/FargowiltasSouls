...
namespace FargowiltasSouls.NPCs.Challengers
{
    ...
    public class LifeChallenger : ModNPC
    {
        ...
        public override void SendExtraAI(BinaryWriter writer)
        {
            ...
            writer.Write(index);
            writer.Write(index2);
            writer.Write(P1state);
            writer.Write(oldP1state);
            writer.Write(LifeWaveCount);
        }
        ...
        public override void ReceiveExtraAI(BinaryReader reader)
        {
            ...
            P1state = reader.ReadInt32();
            oldP1state = reader.ReadInt32();
            LifeWaveCount = reader.ReadInt32();
        }
        #endregion
        #region AI
        ...
        public void P1AI()
        {
            ...
            if (dustcounter > 5)
            {
                for (int j = 0; j < 180; j++)
                {
                    double rad = 2.0 * j * (Math.PI / 180.0);
                    int DustX = (int)NPC.Center.X - (int)(Math.Cos(rad) * threshold);
                    int DustY = (int)NPC.Center.Y - (int)(Math.Sin(rad) * threshold);
        ...
        public void P1ShotSpam()
        {
            ...
            if (NPC.ai[1] > 60f && (NPC.ai[2] >= 20f || (NPC.ai[2] >= 5f && Main.hardMode)))
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int i = -1; i < 2; i++)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, shootatPlayer2.RotatedBy(i * Math.PI / 5), ModContent.ProjectileType<LifeSplittingProjSmall>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.damage), 3f, Main.myPlayer);
                    }
                    NPC.ai[2] = 0f;
                }
            }
            ...
            if (NPC.ai[2] >= 10f && NPC.ai[1] < 130f)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    for (int i = -1; i < 2; i += 2) //-1 and 1
                    {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), SoulPos, soulVel, ModContent.ProjectileType<LifeHomingProj>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.damage), 3f, Main.myPlayer, 0, npcIndex);
        ...
        public void AttackP3Start()
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && NPC.ai[1] < fadeintime)
            {
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(0f, -1f),
                                ModContent.ProjectileType<LifeChalDeathray>(), 0, 0f, Main.myPlayer, /*RotDirSpeed <= 0 ? -(float)rotspeed : (float)rotspeed*/0, NPC.whoAmI);
            }
            if (Main.netMode != NetmodeID.MultiplayerClient && NPC.ai[1] >= fadeintime)
            {
                if (rotspeed < 1.1f)
                ...
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, rotV,
                                ModContent.ProjectileType<LifeChalDeathray>(), 0, 0f, Main.myPlayer, /*RotDirSpeed <= 0 ? -(float)rotspeed : (float)rotspeed*/0, NPC.whoAmI); //no damage because InvisibleScytheHitbox handles that (to override iframes)
        ...
        {
            Player Player = Main.player[NPC.target];
            if (AttackF1) //first he silences you and gets 99999 defense
            {
                AttackF1 = false;
        ...
        public void AttackCharge()
        {
            ...
            if (PhaseThree) //tp
            {
                ...
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), new Vector2(TpPos.X + (float)(NPC.width / 2), TpPos.Y + (float)(NPC.height / 2)), Vector2.Zero, ModContent.ProjectileType<LifeTpTelegraph>(), 0, 0f, Main.myPlayer);
                }
                if (NPC.ai[1] == 75f) //tp
                {
                    NPC.position.X = TpX;
        ...
        public void AttackPlunge()
        {
            ...
            {
                Projectile.NewProjectile(NPC.GetSource_FromThis(), new Vector2(TpPos2.X + (float)(NPC.width / 2), TpPos2.Y + (float)(NPC.height / 2)), Vector2.Zero, ModContent.ProjectileType<LifeTpTelegraph>(), 0, 0f, Main.myPlayer);
            ...
            if (!PhaseThree) //p2 version
            {
                if (NPC.ai[2] > 60f && (NPC.ai[2] % 5) == 0)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, shootoffset4, ModContent.ProjectileType<LifeHomingProj>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.damage), knockBack10, Main.myPlayer, 0f, npcIndex);
                    }
                }
                NPC.ai[2] += 1f;
                if (NPC.ai[1] > 280f)
        ...
        public void AttackRoulette()
        {
            ...
            if (NPC.ai[1] < 40 && Main.netMode != NetmodeID.MultiplayerClient)
            {
                Projectile.NewProjectile(NPC.GetSource_FromThis(), RouletteTpPos, Vector2.Zero, ModContent.ProjectileType<LifeTpTelegraph>(), 0, 0f, Main.myPlayer);
            }
                ...
                switch (choice)
                {
                    ...
                    case 3: //dark souls
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -offset1, ModContent.ProjectileType<LifeHomingProj>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.damage), 3f, Main.myPlayer, 0f, npcIndex);
                        }
                        break;
                }
            }
        ...
        public void AttackTeleportNukes()
        {
            ...
            {
                Projectile.NewProjectile(NPC.GetSource_FromThis(), new Vector2(LockVector1.X + (float)(NPC.width / 2), LockVector1.Y + (float)(NPC.height / 2)), Vector2.Zero, ModContent.ProjectileType<LifeTpTelegraph>(), 0, 0f, Main.myPlayer);
            }
            if (NPC.ai[1] == 60)
            {
        ...
        {
            damage /= 5;
            if (!resigned && NPC.life < NPC.lifeMax / 10 - 1) //works for transition 1, 2 and 3
            {
		...
		public void RandomizeState() //it's done this way so it cycles between attacks in a random order: for increased variety
		{
			...
			if (!PhaseThree && NPC.life < NPC.lifeMax / 10)
			{
				state = 100;
			}
			if (PhaseThree && NPC.life < NPC.lifeMax / 10)
