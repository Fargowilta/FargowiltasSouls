    ...
    public class Retinazer : EModeNPCBehaviour
    {
            ...
            if (FargoSoulsWorld.MasochistModeReal && spazmatism == null && npc.HasValidTarget && ++RespawnTimer > 600)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    if (n != Main.maxNPCs)
                    {
                        ...
                        string text = Language.GetTextValue($"Mods.{Mod.Name}.Message.TwinsRevive", Main.npc[n].FullName);
                        FargoSoulsUtil.PrintText(text, new Color(175, 75, 255));
                    }
                }
            }
        ...
        public override bool CheckDead(NPC npc)
        {
            ...
            if (FargoSoulsUtil.BossIsAlive(ref EModeGlobalNPC.spazBoss, NPCID.Spazmatism) && Main.npc[EModeGlobalNPC.spazBoss].life > 1) //spaz still active
            {
                ...
                {
                    HasSaidEndure = true;
                    string text = Language.GetTextValue($"Mods.{Mod.Name}.Message.TwinsEndure", npc.FullName);
                    FargoSoulsUtil.PrintText(text, new Color(175, 75, 255));
                }
                return false;
            }
