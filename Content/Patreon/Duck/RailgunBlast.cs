    ...
    public class RailgunBlast : MutantSpecialDeathray
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] == 0f)
            {
                if (!Main.dedServ)
                {
                    SoundEngine.PlaySound(new SoundStyle("FargowiltasSouls/Assets/Sounds/Railgun") with {Volume = 0.7f}, Projectile.Center);
                    SoundEngine.PlaySound(new SoundStyle("FargowiltasSouls/Assets/Sounds/Thunder") with {Volume = 0.7f}, player.Center + Projectile.velocity * Math.Min(Main.screenWidth / 2, 900f));
                }
