	...
	public class TrueEyeR : ModProjectile
	{
		...
		public override void AI()
		{
							...
							if (Projectile.localAI[0] == 1f) //spawn orb ring
							{
								...
								for (int i = 0; i < max; i++)
								{
									...
									if (Projectile.owner == Main.myPlayer)
										Projectile.NewProjectile(Projectile.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<PhantasmalSphereTrueEye>(),
											Projectile.damage / 3 * 11, 10f, Projectile.owner, Projectile.identity, i);
								}
							}
				...
				else //forget target
				{
					...
					else //try to recover while making balls
					{
						...
						for (int i = 0; i < 200; i++)
						{
							...
							if (target.CanBeChasedBy())// && Collision.CanHitLine(Projectile.Center, 0, 0, target.Center, 0, 0))
							{
			...
			else
			{
				...
				Vector2 vector2_1 = new(player.direction * 100, 0); //vanilla movement code
